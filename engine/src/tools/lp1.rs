...
// ============================
// Diff-style formatting helpers
// ============================

/// Render an LP1 file patch as a unified-diff-style string.
///
/// Notes:
/// - This output is intended for **human display** (e.g. coloring `-` lines red
///   and `+` lines green in the UI).
/// - It is not guaranteed to be a valid patch that can be applied by external
///   tools like `git apply`, because LP1 does not carry stable hunk line ranges.
#[must_use]
pub fn format_file_patch_as_diff(file: &FilePatch, existed_before: bool) -> String {
    ...
    for op in &file.ops {
        ...
        match op {
            Op::Replace { find, replace, .. } => {
                push_prefixed(&mut out, '-', find);
                push_prefixed(&mut out, '+', replace);
            }
            Op::InsertAfter { find, insert, .. } => {
                push_prefixed(&mut out, ' ', find);
                push_prefixed(&mut out, '+', insert);
            }
            ...
            Op::Append { block } | Op::Prepend { block } => {
                push_prefixed(&mut out, '+', block);
            }
            Op::SetFinalNewline(value) => {
                let flag = if *value { "+" } else { "-" };
                out.push_str(&format!("\\ No final newline flag: {flag}\n"));
            }
        }
    }
...
fn push_prefixed(out: &mut String, prefix: char, lines: &[String]) {
    for line in lines {
        out.push(prefix);
        out.push_str(line);
        out.push('\n');
...
fn op_summary(op: &Op) -> String {
    match op {
        Op::Replace { occ, .. } => format!("LP1 Replace{}", occ_suffix(*occ)),
        Op::InsertAfter { occ, .. } => format!("LP1 InsertAfter{}", occ_suffix(*occ)),
        Op::InsertBefore { occ, .. } => format!("LP1 InsertBefore{}", occ_suffix(*occ)),
        Op::Erase { occ, .. } => format!("LP1 Erase{}", occ_suffix(*occ)),
        Op::Append { .. } => "LP1 Append".to_string(),
        Op::Prepend { .. } => "LP1 Prepend".to_string(),
        Op::SetFinalNewline(value) => {
            let flag = if *value { "+" } else { "-" };
            format!("LP1 SetFinalNewline({flag})")
        }
    }
}

fn occ_suffix(occ: Option<usize>) -> String {
    occ.map_or_else(String::new, |n| format!(" (occ={n})"))
}
