...
- **Input State Machine:** `Normal`, `Insert` (with `DraftInput`), `Command`, `ModelSelect`, `FileSelect`.
...
- **`Message`**: Proper sum type (`User`, `Assistant`, `System`, `Thinking`, `ToolUse`, `ToolResult`).
